...
# with open("face_web_block.png", "wb") as fp:
#     fp.write(result)
...
def face_web():
    ...
    while (time.time() - t_0) < 30:
        screen('screen.png')
        if (recog := rmsdiff('face_web_block.png', 'screen.png')) > 15 and (time.time() - t_0) < 10:
        ...
        #     break
            ...
            print('Facebook web-версия успешно загруженно')
            break
        elif (time.time() - t_0) > 15 and recog>30:
            print('Facebook web-версия не загруженно')
            break
...
def face_app():
    ...
    while (time.time() - t_0) < 15:
        screen('screen.png')
        if (recog := rmsdiff('face_app_block.png', 'screen.png')) < 40:
            result[2] = True
...
def save_vpn():
    with open(f'group_{group + 1}_master.txt', mode='a+', encoding='utf-8') as master:
        while True:
            if len(device.get_top_activities())==2:
                print(vpn_pack:=device.get_top_activities()[1])
                break
        print(f'{vpn_pack.package}/{vpn_pack.activity}', file=master)
    return vpn_pack

print(device.shell('dumpsys netstats details full'))
while True:
    group = 2  # Номер группы тестирования VPN-ов +1
    vpn=save_vpn()
